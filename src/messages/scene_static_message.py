...
class SceneLaneSegment(PUBSUB_MSG_IMPL):
    ...
    def deserialize(cls, pubsubMsg: TsSYSSceneLaneSegment):
        ...
        pubsubMsg.e_Cnt_left_boundary_points_count = max(len(pubsubMsg.as_left_boundary_points), 32)
        pubsubMsg.e_Cnt_right_boundary_points_count = max(len(pubsubMsg.as_right_boundary_points), 32)
