...
def main():
    ...
    modules_list = \
        [
            DmProcess(create_state_module(),
                      trigger_type=DmTriggerType.DM_TRIGGER_NONE,
                      trigger_args={}),

            DmProcess(create_behavioral_planner(),
                      trigger_type=DmTriggerType.DM_TRIGGER_PERIODIC,
                      trigger_args={'period': BEHAVIORAL_PLANNING_MODULE_PERIOD}),

            DmProcess(create_trajectory_planner(),
                      trigger_type=DmTriggerType.DM_TRIGGER_PERIODIC,
                      trigger_args={'period': TRAJECTORY_PLANNING_MODULE_PERIOD})

            #DmProcess(create_navigation_planner(),
            #          trigger_type=DmTriggerType.DM_TRIGGER_NONE,
            #          trigger_args={}),
        ]
    manager = DmManager(modules_list)
