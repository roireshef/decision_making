...
class WerlingPlanner(TrajectoryPlanner):
    ...
    def plan(self, state: State, reference_route: FrenetSerret2DFrame, goal: CartesianExtendedState, T: float,
        ...
        if len(ctrajectories_filtered) == 0:
            ...
            best_lat_acc_traj_idx = np.argmin(np.max(np.abs(lat_acc), axis=1))
            worse_lat_acc_time_idx = np.argmax(np.abs(lat_acc[best_lat_acc_traj_idx]))
            worse_lat_acc_vel = ctrajectories[best_lat_acc_traj_idx, worse_lat_acc_time_idx, C_V]
            worse_lat_acc_k = ctrajectories[best_lat_acc_traj_idx, worse_lat_acc_time_idx, C_K]
            raise CartesianLimitsViolated("Cartesian Limits Violation - No valid trajectories. "
                                          ...
                                          "planned lat. accelerations range [%s, %s] (limits: %s), t=%.1f v=%.3f k=%f\n"
                                          ...
                                          (state.ego_state.timestamp_in_sec, T, planning_horizon,
                                           ...
                                           WERLING_TIME_RESOLUTION*worse_lat_acc_time_idx, worse_lat_acc_vel, worse_lat_acc_k,
        ...
        if is_target_ahead:
            ...
            pos_s = ftrajectories[cartesian_filtered_indices[sorted_filtered_idxs[0]], 0, FS_SX]
            vel_s = ftrajectories[cartesian_filtered_indices[sorted_filtered_idxs[0]], 0, FS_SV]
            pos_d = ftrajectories[cartesian_filtered_indices[sorted_filtered_idxs[0]], 0, FS_DX]
            vel_d = ftrajectories[cartesian_filtered_indices[sorted_filtered_idxs[0]], 0, FS_DV]
            poly_s = np.array([0, 0, 0, 0, vel_s, pos_s])
            poly_d = np.array([0, 0, 0, 0, vel_d, pos_d])
