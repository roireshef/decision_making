...
class OptimalControlUtils:
    ...
    class QuarticPoly1D(Poly1D):
        ...
        def polyval_with_derivatives(poly_coefs: np.ndarray, time_samples: np.ndarray) -> np.ndarray:
            """
            For each (quartic) position polynomial(s) and time-sample it generates 3 values:
        ...
        def time_constraints_matrix(T: float) -> np.ndarray:
            """
            Given the quartic polynomial setting, this function returns A for a specific value of T
            :param T:
            :return:
            """
            return OptimalControlUtils.QuarticPoly1D.time_constraints_tensor(np.array([T]))[0]

        @staticmethod
        def is_acceleration_in_limits(poly_coefs: np.ndarray, T: float, acc_limits: Limits) -> bool:
            """
            given coefficients vector of a quartic polynomial x(t), and restrictions
            ...
            """
            return OptimalControlUtils.QuarticPoly1D.are_accelerations_in_limits(np.array([poly_coefs]),
                                                                                 np.array([T]), acc_limits)[0]

        @staticmethod
        def are_accelerations_in_limits(poly_coefs: np.ndarray, T_vals: np.ndarray, acc_limits: Limits) -> np.ndarray:
            """
            Applies the following on a vector of polynomials and planning-times: given coefficients vector of a quartic
            polynomial x(t), and restrictions on the acceleration values, return True if restrictions are met,
            False otherwise
            :param polys_coefs: 2D numpy array with N polynomials and 5 coefficients each [Nx5]
            ...
            # compute extrema points, by finding the roots of the 3rd derivative (which is itself a 1st degree polynomial)
            jerk_poly = Math.polyder2d(poly_coefs, m=3)
            acc_poly = Math.polyder2d(poly_coefs, m=2)

            # TODO: this was taken from QuinticPoly1D (modified - should be tested!!!).
