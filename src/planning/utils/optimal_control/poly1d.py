...
class QuinticPoly1D(Poly1D):
    ...
    def distance_from_target(a_0: float, v_0: float, v_T: float, ds0: float, T: float, T_m: float):
        ...
        travelled since time 0
        """
        return lambda t: (-T ** 5 * t * (a_0 * t + 2 * v_0) + 2 * T ** 5 * (ds0 + t * v_T) + T ** 2 * t ** 3 * (
                    3 * T ** 2 * a_0 + 4 * T * (3 * v_0 + 2 * v_T)
                    - 20 * ds0 - 20 * v_T * (T - T_m)) - T * t ** 4 * (
                                      3 * T ** 2 * a_0 + 2 * T * (8 * v_0 + 7 * v_T) - 30 * ds0 - 30 * v_T * (T - T_m))
                          + t ** 5 * (T ** 2 * a_0 + 6 * T * (v_0 + v_T) - 12 * ds0 - 12 * v_T * (T - T_m))) / (
                                     2 * T ** 5)
    ...
    @staticmethod
    def velocity_profile_derivative_coefs(a_0: float, v_0: float, v_T: float, ds: float, T: float, T_m: float):
        """
        ...
        :return: lambda function(s) that takes relative time in seconds and returns the velocity
        """
        coefs = np.array([10 * (-T ** 2 * a_0 - 6 * T * (v_0 + v_T) + 12 * ds + 12 * v_T * (T - T_m)),
                          + 6 * T * (3 * T ** 2 * a_0 + 2 * T * (8 * v_0 + 7 * v_T) - 30 * ds - 30 * v_T * (T - T_m)),
                          - 3 * T ** 2 * (
    ...
    def acceleration_profile_function(a_0: float, v_0: float, v_T: float, ds: float, T: float, T_m: float):
        ...
        """
        return lambda t: np.inner(
            QuinticPoly1D.velocity_profile_derivative_coefs(a_0, v_0, v_T, ds, T, T_m),
            np.array([t ** 3, t ** 2, t, 1]))

    @staticmethod
    def acceleration_profile_derivative_coefs(a_0: float, v_0: float, v_T: float, ds: float, T: float, T_m: float):
        """
        coefficients of the derivative of the acceleration profile
        :param a_0: [m/sec^2] acceleration at time 0
        :param v_0: [m/sec] velocity at time 0
        ...
        :return: lambda function(s) that takes relative time in seconds and returns the velocity
        """
        coefs = np.array([30 * (-T ** 2 * a_0 - 6 * T * (v_0 + v_T) + 12 * ds + 12 * v_T * (T - T_m)),
                          + 12 * T * (3 * T ** 2 * a_0 + 2 * T * (8 * v_0 + 7 * v_T) - 30 * ds - 30 * v_T * (T - T_m)),
                          - 3 * T ** 2 * (3 * T ** 2 * a_0 + 4 * T * (3 * v_0 + 2 * v_T) - 20 * ds - 20 * v_T * (T - T_m))]) / T ** 5
        return coefs
