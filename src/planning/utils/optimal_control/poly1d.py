...
class QuinticPoly1D(Poly1D):
    ...
    @staticmethod
    def time_cost_function_derivative_coefs_front_rss(w_T: np.array, w_J: np.array, a_0: np.array, v_0: np.array,
                                                      v_T: np.array, v_f: np.array, rel_s: np.array, T_m: float,
                                                      a_br: float):
        """
        For given weights and constraints on a jerk-optimal polynomial solution, this function returns a matrix that
        contains (in each row:) the coefficients of the derivative of the cost function use for finding the optimal time
        horizon. Assumptions:
            Target velocity v_T is different from the front actor's velocity v_f.
            Ego terminal s is at RSS-safe distance from the front actor.
        f(T) = w_T * T + w_J * J(T) where J(T) is the accumulated jerk for given time horizon T.
        :param w_T: weight for Time component
        :param w_J: weight for Jerk component
        :param a_0: [m/sec^2] acceleration at time 0
        :param v_0: [m/sec] velocity at time 0
        :param v_T: [m/sec] terminal velocity (at time T)
        :param v_f: [m/sec] front actor's velocity
        :param rel_s: [m] s of front actor relative to ego
        :param T_m: T_m: [sec] reaction time of ego
    ...
    def time_cost_function_derivative_coefs_back_rss(w_T: np.array, w_J: np.array, a_0: np.array, v_0: np.array,
                                                     v_T: np.array, v_b: np.array, rel_s: np.array, T_m: float,
                                                     a_br: float):
        """
        For given weights and constraints on a jerk-optimal polynomial solution, this function returns a matrix that
        contains (in each row:) the coefficients of the derivative of the cost function use for finding the optimal time
        horizon. Assumptions:
            Target velocity v_T is different from the back actor's velocity v_b.
            Ego terminal s is at RSS-safe distance from the back actor.
        f(T) = w_T * T + w_J * J(T) where J(T) is the accumulated jerk for given time horizon T.
        :param w_T: weight for Time component
        :param w_J: weight for Jerk component
        :param a_0: [m/sec^2] acceleration at time 0
        :param v_0: [m/sec] velocity at time 0
        :param v_T: [m/sec] terminal velocity (at time T)
        :param v_b: [m/sec] back actor's velocity
        :param rel_s: [m] s of front actor relative to ego
        :param T_m: T_m: [sec] reaction time of ego
        :param a_br: [m/sec^2] maximal deceleration of ego & actor (RSS)
        :return: coefficient matrix for all possibilities
