...
class BehavioralGridState:
    ...
    def _create_generalized_frenet_frames(state: State, route_plan: RoutePlan, logger: Logger) -> \
        ...
        '''
        TODO: Remove the two if statements below when there is no longer the possibility of having identical GFFs. Without these statements,
        identical GFFs will occur when a lane split in the host vehicle's lane is within the backward horizon that is used to create GFFs.
        While creating GFFs, the upstream lane segments are iterated over until the backward horizon is reached, and if a lane split is
        present, the upstream lane segment will be the same for the relevant lanes. This results in multiple GFFs being created from the
        same location. When the downstream lane segments are iterated over for each GFF, the downstream lane segment that is chosen at the
        lane split is based on the RP lane end cost. Therefore, the same downstream lane is chosen for each GFF, and this results in
        identical GFFs. One reason why this is an issue is that dynamic objects can be assigned to the wrong lane. This was discovered
        during the addition of lane split functionality.
        '''
