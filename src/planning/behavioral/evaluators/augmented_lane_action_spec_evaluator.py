    ...
    def evaluate(self, behavioral_state: BehavioralGridState, action_recipes: List[ActionRecipe],
        ...
        * If no action from the previous bullet is found valid, find the ActionType.FOLLOW_LANE action with maximal
        ...
        for target_lane in lanes_to_try:
            ...
            # last, look for valid static action closest to desired velocity
            desired_velocity = self.get_desired_velocity_for_lane(target_lane, behavioral_state)
            selected_follow_lane_idx = self._get_follow_lane_valid_action_idx(action_recipes, action_specs, action_specs_mask, target_lane,
                                                                              desired_velocity)
