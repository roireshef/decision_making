...
class BehavioralPlanningFacade(DmModule):
    ...
    def _stop_impl(self):
        ...
        self.pubsub.unsubscribe(pubsub_topics.PubSubMessageTypes["UC_SYSTEM_NAVIGATION_PLAN_LCM"])
    ...
    def _get_current_navigation_plan(self) -> NavigationPlanMsg:
        ...
        is_success, serialized_nav_plan = self.pubsub.get_latest_sample(topic=topic, timeout=1.0)
        if (not is_success) or (serialized_nav_plan is None):
            raise MsgDeserializationError(
                "Pubsub message queue for %s topic is empty or topic isn\'t subscribed" % topic)
