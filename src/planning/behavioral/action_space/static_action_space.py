...
from decision_making.src.global_constants import BP_ACTION_T_LIMITS, BP_JERK_S_JERK_D_TIME_WEIGHTS_FOLLOW_LANE, \
    VELOCITY_LIMITS
...
class StaticActionSpace(ActionSpace):
    ...
    def specify_goals(self, action_recipes: List[StaticActionRecipe], behavioral_state: BehavioralGridState) -> \
        ...
        weights = BP_JERK_S_JERK_D_TIME_WEIGHTS_FOLLOW_LANE[aggressiveness]
        ...
        cost_coeffs_d = QuinticPoly1D.time_cost_function_derivative_coefs(
            w_T=weights[:, 2], w_J=weights[:, 1], a_0=projected_ego_fstates[:, FS_DA], v_0=projected_ego_fstates[:, FS_DV],
            v_T=0, dx=-projected_ego_fstates[:, FS_DX], T_m=0)
