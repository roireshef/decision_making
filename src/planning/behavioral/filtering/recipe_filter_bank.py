...
class FilterActionsTowardsNonOccupiedCells(RecipeFilter):
    def filter(self, recipes: List[DynamicActionRecipe], behavioral_state: BehavioralGridState, logger: Optional[Logger] = None) -> List[bool]:
...
class FilterActionsTowardsCellsWithoutStopSignsOrStopBars(RecipeFilter):
    def filter(self, recipes: List[RoadSignActionRecipe], behavioral_state: BehavioralGridState, logger: Optional[Logger] = None) -> List[bool]:
        return [len(MapUtils.get_stop_bar_and_stop_sign(
            behavioral_state.extended_lane_frames[recipe.relative_lane], logger)) > 0
...
class FilterSpeedingOverDesiredVelocityStatic(RecipeFilter):
    ...
    """
    def filter(self, recipes: List[StaticActionRecipe], behavioral_state: BehavioralGridState, logger: Optional[Logger] = None) -> List[bool]:
