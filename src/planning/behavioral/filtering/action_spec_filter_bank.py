...
class FilterForSafetyTowardsTargetVehicle(ActionSpecFilter):
    ...
    def _check_safety_for_actor(behavioral_state: BehavioralGridState, ego_ftrajectories: FrenetTrajectories2D,
        ...
        safety_dist = FilterForSafetyTowardsTargetVehicle._get_lon_safe_dist(
            ...
            obj_ftrajectory, SAFETY_HEADWAY, time_ranges[:, 1] - min_from_time_idx, margin,
            actor_cell[1] == RelativeLongitudinalPosition.FRONT, logger)

        # zero all safety_dist beyond trajectory_lengths
        max_len = max_till_time_idx - min_from_time_idx
        aux_matrix = np.full((ego_ftrajectories.shape[0], max_len), np.arange(max_len))
        safety_mask = (aux_matrix < time_ranges[:, 1, np.newaxis] - min_from_time_idx) & \
                      (aux_matrix >= time_ranges[:, 0, np.newaxis] - min_from_time_idx)

        return np.min(safety_dist * safety_mask, axis=1)  # minimum on time
    ...
    def _get_lon_safe_dist(ego_trajectories: FrenetTrajectories2D, ego_response_time: float,
                           ...
                           till_time_idx: np.array, margin: float, front_actor: bool, logger: Logger,
        ...
        ego_s, ego_v, ego_lat = ego_trajectories[..., FS_SX], ego_trajectories[..., FS_SV], ego_trajectories[..., FS_DX]
        ...
        if front_actor:
            ...
            for i in range(delay_shift):
                ext_ego_s[range(ext_ego_s.shape[0]), till_time_idx + i] = ego_extrapolation[:, i, FS_SX]
                ext_ego_v[range(ext_ego_v.shape[0]), till_time_idx + i] = ego_extrapolation[:, i, FS_SV]
