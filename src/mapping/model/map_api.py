...
from decision_making.src.mapping.model.constants import DEFAULT_MAP_LAYER, MAX_STEPS_FOR_ROAD_BASED_NAVIGATION_PLAN
from decision_making.src.mapping.model.localization import RoadCoordinatesDifference, RoadLocalization
from decision_making.src.mapping.model.map_model import RoadDetails, MapModel
from decision_making.src.mapping.transformations.geometry_utils import CartesianFrame, Euclidean
from decision_making.src.messages.navigation_plan_message import NavigationPlanMsg
from decision_making.src.planning.types import FP_SX, FP_DX
from decision_making.src.planning.utils.frenet_serret_frame import FrenetSerret2DFrame
from decision_making.src.map_exceptions import raises, RoadNotFound, MapCellNotFound, LongitudeOutOfRoad, OutOfSegmentBack, \
    OutOfSegmentFront, NextRoadNotFound, LaneNotFound
...
class MapAPI:
    ...
    def convert_geo_to_map_coordinates(self, lat, lon, layer=0):
        # type: (float, float, int) -> Tuple[float, float, Tuple(int,str)]
        """
    ...
    def get_road(self, road_id):
        ...
        return copy.deepcopy(self.__get_road(road_id))

    @raises(RoadNotFound)
    def get_num_lanes(self, road_id):
        # type: (int) -> int
        return self.__get_road(road_id).lanes_num
    ...
    def get_uniform_path_lookahead(self, road_id, lat_shift, starting_lon, lon_step, steps_num, navigation_plan):
        ...
        """
        shifted, _ = self.get_lookahead_points(road_id, starting_lon, lon_step * steps_num, lat_shift, navigation_plan)
    ...
    def convert_road_to_global_coordinates(self, road_id, lon, lat):
        ...
        """
        frenet = self._rhs_roads_frenet[road_id]
    ...
    def get_lane_width(self, road_id, lane_num, road_lon):
        ...
        :param road_id: The required road id
        ...
        if lane_num < 0 or lane_num >= road._lanes_num:
            raise LaneNotFound('Unknown road with id %s' % road_id)
    ...
    def _dist_to_road(self, x, y, road_id):
        ...
        :param x: x coordinate on map (given in [m])
        :param y: y coordinate on map (given in [m])
        :param road_id: the road id to measure distance to
    ...
    def _convert_global_to_road_coordinates(self, x, y, yaw, road_id):
        ...
        try:
            ...
            raise LongitudeOutOfRoad("LongitudeOutOfRoad: Tried to project point %s onto road #%s but projection "
                                     "falls outside the road (longitudinally)" % (point, road_id))
    ...
    def get_road_based_navigation_plan(self, current_road_id,
                                       max_steps=MAX_STEPS_FOR_ROAD_BASED_NAVIGATION_PLAN):
    ...
    def get_road_center_frenet_frame(self, road_id: int) -> FrenetSerret2DFrame:
        """
        ...
        return self._roads_frenet[road_id]
