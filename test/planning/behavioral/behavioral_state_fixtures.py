...
@pytest.fixture(scope='function')
def state_for_testing_lanes_speed_limits_violations():
...
def state_for_testing_lanes_speed_limits_violations():
    ...
    map_state = MapState(np.array([ego_lane_lon, ego_vel, 0, 0, 0, 0]), lane_id)
    ego_state = EgoState.create_from_map_state(obj_id=0, timestamp=0, map_state=map_state, size=car_size,
                                               confidence=1, off_map=False)
