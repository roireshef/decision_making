...
def test_novDemoEvalSemanticActions(testable_map_api):
    ...
    actions_spec.append(SemanticActionSpec(t=2, v=vel2, s_rel=obs[1].x-ego.x, d_rel=obs[1].y-ego.y))
    actions_spec.append(SemanticActionSpec(t=5, v=vel3, s_rel=obs[2].x-ego.x, d_rel=obs[2].y-ego.y))

    costs = NovDemoPolicy._eval_actions(state, semantic_actions, actions_spec, max_velocity)
    print(costs)
