...
def init_test():
    ...
    return v_des, ego_lon, lane_width, car_length, size, action_space, spec_evaluator, action_spec_validator, \
           value_approximator, planner, road_frenet


def evaluate_action_state(ego: EgoState, objects: List[DynamicObject], action_space: ActionSpace, logger: Logger,
                          action_spec_validator: ActionSpecFiltering, spec_evaluator: HeuristicActionSpecEvaluator,
                          planner: SingleStepBehavioralPlanner, value_approximator: NaiveValueApproximator) -> \
    ...
    terminal_behavioral_states = planner._generate_terminal_states(state, list(specs), np.array(specs_mask))
    ...
    return selected_lane, best_idx, recipes[best_idx].relative_lane.value, costs[best_idx], terminal_states_values[best_idx]
    ...
    for ego_vel in ego_vel_range:
        for F_vel in F_vel_range:
            for TC_F in TC_F_range:
                ...
                F_lon = ego_lon + calc_init_dist_by_safe_time(True, ego_vel, F_vel, TC_F, SAFETY_MARGIN_TIME_DELAY, car_length)
                if np.isinf(F_lon):
                    continue
                ...
                selected_lane, best_idx, rel_lane, best_cost, best_val = evaluate_action_state(
                    ego, [F], action_space, logger, action_spec_validator, spec_evaluator, planner, value_approximator)

                print('ego_vel=%.2f F_vel=%.2f TC_F=%.2f dist=%.2f cost=%.2f val=%.2f; i=%d lane=%d\n' %
                      (ego_vel, F_vel, TC_F, F_lon-ego_lon, best_cost, best_val, best_idx, selected_lane))
    ...
    for LF_vel in LF_vel_range:
        for TC_LF in TC_LF_range:
            for F_vel in F_vel_range:
                ...
                selected_lane, best_idx, rel_lane, best_cost, best_val = evaluate_action_state(
                    ego, [F, LF], action_space, logger, action_spec_validator, spec_evaluator, planner, value_approximator)
...
def test_evaluate_differentStatesLBandF_laneChangeAccordingToTheLogic():
    ...
    for F_vel in F_vel_range:
        for TC_LB in TC_LB_range:
            ...
            LB_lon = ego_lon - calc_init_dist_by_safe_time(False, ego_vel, LB_vel, TC_LB, SPECIFICATION_MARGIN_TIME_DELAY, car_length)
            if np.isinf(LB_lon):
                continue
            LB = create_canonic_object(2, 0, LB_lon, 3 * lane_width / 2, LB_vel, size, road_frenet)

            selected_lane, best_idx, rel_lane, best_cost, best_val = evaluate_action_state(
                ego, [F, LB], action_space, logger, action_spec_validator, spec_evaluator, planner, value_approximator)
...
def test_evaluate_differentDistancesAndVeloctiesOfFandRF_laneChangeAccordingToTheLogic():
    ...
    for TC_RF in sec_to_RF_range:
        for RF_vel in RF_vel_range:
            ...
            selected_lane, best_idx, rel_lane, best_cost, best_val = evaluate_action_state(
                ego, [RF], action_space, logger, action_spec_validator, spec_evaluator, planner, value_approximator)
