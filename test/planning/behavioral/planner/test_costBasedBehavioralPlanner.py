...
def test_generateTerminalStates_multiRoad():
    ...
    # ego = EgoState.create_from_cartesian_state(obj_id=0, timestamp=timestamp, cartesian_state=ego_cstate, size=size, confidence=0)
    #
    # obj1_cstate = MapUtils.get_lane_frenet_frame(201).fstate_to_cstate(np.array([80, 14, 0, 0, 0, 0]))
    # obj1 = EgoState.create_from_cartesian_state(obj_id=1, timestamp=timestamp, cartesian_state=obj1_cstate, size=size, confidence=0)
    #
    # obj2_cstate = MapUtils.get_lane_frenet_frame(200).fstate_to_cstate(np.array([70, 10, 0, 0, 0, 0]))
    # obj2 = EgoState.create_from_cartesian_state(obj_id=2, timestamp=timestamp, cartesian_state=obj2_cstate, size=size, confidence=0)
    #
    # obj3_cstate = MapUtils.get_lane_frenet_frame(202).fstate_to_cstate(np.array([90, 16, 0, 0, 0, 0]))
    # obj3 = EgoState.create_from_cartesian_state(obj_id=3, timestamp=timestamp, cartesian_state=obj3_cstate, size=size, confidence=0)
    #
    # obj4_cstate = MapUtils.get_lane_frenet_frame(211).fstate_to_cstate(np.array([5, 18, 0, 0, 0, 0]))
    # obj4 = EgoState.create_from_cartesian_state(obj_id=4, timestamp=timestamp, cartesian_state=obj4_cstate, size=size, confidence=0)
    #
    # obj5_cstate = MapUtils.get_lane_frenet_frame(212).fstate_to_cstate(np.array([10, 15, 0, 0, 0, 0]))
    # obj5 = EgoState.create_from_cartesian_state(obj_id=5, timestamp=timestamp, cartesian_state=obj5_cstate, size=size, confidence=0)
    #
    # obj6_cstate = MapUtils.get_lane_frenet_frame(200).fstate_to_cstate(np.array([30, 15, 0, 0, 0, 0]))
    # obj6 = EgoState.create_from_cartesian_state(obj_id=6, timestamp=timestamp, cartesian_state=obj6_cstate, size=size, confidence=0)
    #
    ...
    #
    # planner = SingleStepBehavioralPlanner(action_space, recipe_evaluator, action_spec_evaluator, action_spec_filtering,
    ...
    # valid_action_recipes = [action_recipe for i, action_recipe in enumerate(action_space.recipes) if recipes_mask[i]]
    #
    ...
    #
    # terminal_behavioral_states = planner._generate_terminal_states(state, behavioral_state, action_specs, action_specs_mask, nav_plan)
    #
    ...
    # assert np.isclose(np.array([state.ego_state.timestamp_in_sec for state in terminal_behavioral_states if state is not None]),
