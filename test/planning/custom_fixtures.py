...
def state_module(state, pubsub):
    ...
    state_mock = StateModuleMock(logger, state)
    state_mock.start()
    yield state_mock
    state_mock.stop()
...
def navigation_facade(pubsub, navigation_plan):
    ...
    navigation_module = NavigationFacadeMock(logger=logger, navigation_plan_msg=navigation_plan)
...
def trajectory_planner_facade(pubsub, trajectory, trajectory_visualization_msg):
    ...
    trajectory_planning_module = TrajectoryPlanningFacadeMock(logger=logger,
