...
def test_convertGlobalToRelativeFrame_onlyTranslation_successful():
    ...
    assert np.linalg.norm(relative_pos - np.array([1, 0, 0])) < DIVISION_FLOATING_ACCURACY
...
def test_convertGlobalToRelativeFrame_withFortyFiveDegRotation_successful():
    ...
    assert np.linalg.norm(relative_pos - np.array([1/np.sqrt(2), -1/np.sqrt(2), 0])) < DIVISION_FLOATING_ACCURACY
...
def test_convertRelativeToGlobalFrame_onlyTranslation_successful():
    ...
    assert np.linalg.norm(relative_pos - np.array([3, 2, 0])) < DIVISION_FLOATING_ACCURACY
...
def test_convertRelativeToGlobalFrame_withFortyFiveDegRotation_successful():
    ...
    assert np.linalg.norm(relative_pos - np.array([1 - 1/np.sqrt(2), 1 + 1/np.sqrt(2), 0])) < DIVISION_FLOATING_ACCURACY
