...
class PlottableSigmoidStatic2DBoxObstacle(SigmoidStatic2DBoxObstacle):
    def plot(self, plt):
        ...
        R_inv = np.linalg.inv(self._R[0].transpose())
        lower_left_p = np.dot(R_inv, [-self.length / 2, -self.width / 2, 1])
        ...
        lower_left_p = np.dot(R_inv, [-self.length / 2 - self._margin, -self.width / 2 - self._margin, 1])
        plt.add_patch(patches.Rectangle(
