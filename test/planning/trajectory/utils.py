...
class PlottableSigmoidStatic2DBoxObstacle(SigmoidStatic2DBoxObstacle):
    def plot(self, plt):
        ...
        lower_left_p = np.dot(self._R, [-self.length / 2, -self.width / 2, 1])
        plt.add_patch(patches.Rectangle(
            (lower_left_p[0], lower_left_p[1]), self.length, self.width, angle=np.rad2deg(self.theta), hatch='\\',
            fill=False
        ))

        lower_left_p = np.dot(self._R, [-self.length / 2 - self._margin, -self.width / 2 - self._margin, 1])
        plt.add_patch(patches.Rectangle(
