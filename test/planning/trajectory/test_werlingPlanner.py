...
def test_werlingPlanner_toyScenario_noException():
    ...
    obs = list([
        DynamicObject(obj_id=0, timestamp=950*10e6, cartesian_state=np.array([pos1[0],pos1[1], yaw1, 0, 0, 0]), map_state=None,
                      size=ObjectSize(1.5, 0.5, 0),
                      confidence=1.0)
        DynamicObject(obj_id=0, timestamp=950*10e6, cartesian_state=np.array([pos2[0], pos2[1], yaw2, 0, 0, 0]), map_state=None, size=ObjectSize(1.5, 0.5, 0),
                      confidence=1.0)
    ])

    # set ego starting longitude > 0 in order to prevent the starting point to be outside the reference route
    ego = EgoState(obj_id=-1, timestamp=1000*10e6, cartesian_state=np.array([LON_MARGIN_FROM_EGO,0,0,0,0,0]), map_state=None,
...
def create_state_for_test_werlingPlanner(frenet: FrenetSerret2DFrame, obs_poses: np.array,
    ...
    ego = EgoState(obj_id=-1, timestamp=0, cartesian_state=np.array([ctraj_start_goal[0][C_X], ctraj_start_goal[0][C_Y],
                 ctraj_start_goal[0][C_YAW],ctraj_start_goal[0][C_V],0,0]),map_state = None, size=ObjectSize(EGO_LENGTH, EGO_WIDTH, 0),
                   confidence=1.0)
    ...
    for i, pose in enumerate(obs_poses):
        ...
        obs.append(DynamicObject(obj_id=i, timestamp=0, cartesian_state=np.array([cobs[C_X], cobs[C_Y],
                                 frenet.get_yaw(pose[FP_SX]),0,0,0]),map_state=None, size=ObjectSize(4, 1.8, 0), confidence=1.0)
