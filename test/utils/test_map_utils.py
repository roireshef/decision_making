...
def test_getDistToLaneBorders_rightLane_equalToHalfLaneWidth(scene_static):
    ...
    #MapService.initialize(MAP_SPLIT)
    #road_ids = MapService.get_instance()._cached_map_model.get_road_ids()
    road_ids = [road_segment.e_Cnt_road_segment_id for road_segment in scene_static.as_scene_road_segment]
    road_ids = MapService.get_instance()._cached_map_model.get_road_ids()
...
def test_getClosestLane_multiLaneRoad_testExceptionOnWrongRoadId(scene_static):
    """
    test method get_closest_lane:
...
def test_doesMapExistBackward_longBackwardDist_validateRelevantException(scene_staic):
    SceneModel.get_instance().add_scene_static(scene_static)
    MapService.initialize(MAP_SPLIT)
    road_segment_ids = MapService.get_instance()._cached_map_model.get_road_ids()
