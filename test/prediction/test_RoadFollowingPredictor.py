...
def test_predictObjectTrajectories_precisePredictionDynamicAndStaticObjectMultipleSingleTimestamp(car_size):
    ...
    dyn_obj = DynamicObject(obj_id=1, timestamp=1e9, x=global_pos[C_X], y=global_pos[C_Y], z=global_pos[C_Z], yaw=0,
                            size=car_size, confidence=0, v_x=10, v_y=0, acceleration_lon=0, omega_yaw=0)
...
def test_predictObject_zeroSpeedZeroLookahead_noException(car_size):
    ...
    predictor = RoadFollowingPredictor(logger)
    global_pos = np.array([500.0, 0.0, 0.0])
    dyn_obj = DynamicObject(obj_id=1, timestamp=0, x=global_pos[C_X], y=global_pos[C_Y], z=global_pos[C_Z], yaw=0,
                            size=car_size,
                            confidence=0,
...
def test_predictObjectOnRoad_precisePrediction():
    ...
    global_pos = np.array([500.0, 0.0, 0.0])
    velocity = 10
    dynamic_object = DynamicObject(obj_id=1, timestamp=0, x=global_pos[C_X], y=global_pos[C_Y], z=global_pos[C_Z],
                                   yaw=0, size=car_size,
...
def test_predictState_precisePrediction(car_size):
    ...
    dyn_obj = DynamicObject(obj_id=1, timestamp=1e9, x=dyn_global_pos[C_X], y=dyn_global_pos[C_Y],
                            z=dyn_global_pos[C_Z], yaw=0, size=car_size,
                            confidence=0, v_x=10, v_y=0, acceleration_lon=0, omega_yaw=0)

    ego_global_pos = np.array([450.0, 0.0, 0.0])
    ego = EgoState(obj_id=0, timestamp=2e9, x=ego_global_pos[C_X], y=ego_global_pos[C_Y], z=ego_global_pos[C_Z], yaw=0,
                   size=car_size,
