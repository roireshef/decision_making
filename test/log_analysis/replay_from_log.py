...
class TrajectoryPlanningFacadeNoLcm(TrajectoryPlanningFacade):
    ...
    def _get_current_state(self) -> State:
        ...
        """
        input_state = self.pubsub.get_latest_sample(topic=pubsub_topics.PubSubMessageTypes["UC_SYSTEM_STATE"], timeout=1)
    ...
    def _get_mission_params(self) -> TrajectoryParams:
        ...
        """
        input_params = self.pubsub.get_latest_sample(topic=pubsub_topics.PubSubMessageTypes["UC_SYSTEM_TRAJECTORY_PARAMS"], timeout=1)
...
def execute_tp(state_serialized: Dict, tp_params_serialized: Dict) -> None:
    ...
    pubsub.publish(pubsub_topics.PubSubMessageTypes["UC_SYSTEM_STATE"], state_serialized)
    pubsub.publish(pubsub_topics.PubSubMessageTypes["UC_SYSTEM_TRAJECTORY_PARAMS"], tp_params_serialized)
